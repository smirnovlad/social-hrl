...
(a) Episode return over training --- only flat PPO makes any progress in this budget; the shaded envelope is the per-seed min/max to keep flat's large variance from crushing the HRL curves.
...
Goal-space coverage & $\mathbf{1.00 \pm 0.00}$ & $0.72 \pm 0.08$ & $0.55 \pm 0.05$ \\
...
The corridor is a weak coordination probe (flat PPO already solves it). To stress the message channel, we defined four scenarios: \textit{baseline} (no forced coordination), \textit{bus} (both agents must pass through one shared cell), \textit{bus\_strict} (as bus, with a tighter 4-step timing window), and \textit{turn\_taking} (agents must alternate priority). Runs are 12k timesteps each, 3 seeds per cell.

\begin{figure}[h]
\centering
\includegraphics[width=0.98\columnwidth]{figs/fig4_rq4_scenarios.pdf}
\caption{\textbf{Goal-space coverage per scenario} (LOLA, 3 seeds, 12k).
Across all four coordination scenarios LOLA coverage sits in the $0.50$--$0.71$ range, with \textit{turn\_taking} slightly higher than the bus variants. Discrete and social aggregated-summary JSONs for this sweep were not preserved in the submission snapshot, so only the LOLA arm renders; their baseline-scenario values ($\sim\!1.00$ and $\sim\!0.72$ respectively) are reported in Figure~\ref{fig:main-story}c instead.
}
\label{fig:rq4}
\end{figure}

The LOLA arm of Figure~\ref{fig:rq4} shows that coverage is roughly flat across the bus and bus\_strict stressors ($\sim\!0.50$--$0.54$) and slightly higher under turn\_taking ($\sim\!0.71$). We had hypothesised that forcing tighter coordination would either (a) destabilise the channel and reduce coverage further, or (b) push the channel toward higher mutual information by demanding more precise messages. Within LOLA, neither happened: the bus-family scenarios are statistically indistinguishable from the baseline. The turn-taking uptick is intriguing but single-arm evidence; it is not enough on its own to claim the compression we observed in Section H2 reverses under coordination pressure. A proper RQ4 conclusion requires rerunning the sweep for discrete and social, which is flagged as future work.
...
\paragraph{Adding a partner does not help.} Across both waves, every coordination-aware variant we tried (vanilla social, LOLA) reduces both coverage and per-message information content relative to the discrete-solo baseline. The information drop is large: $I(\text{goal}; \text{msg})$ falls from $1.58$ nats (discrete) to $0.31$ (social) and $0.33$ (LOLA), and a held-out ridge listener recovers the goal direction with $R^2 = 0.25$ from a discrete message but only $0.05$ from a social one. The LOLA arm of the RQ4 coordination-scenarios sweep (Figure~\ref{fig:rq4}) is consistent with this picture within its single-arm limitation: coverage stays in the $0.5$--$0.7$ range across bus and bus\_strict stressors. The transfer experiment (Figure~\ref{fig:transfer}) is also consistent: social and discrete source managers produce indistinguishable target-eval returns.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Coordination scenarios.} Figure~\ref{fig:rq4} shows the LOLA arm of a 4-scenario coordination sweep; discrete and social aggregated summaries were not preserved in time for this submission, so the cross-mode RQ4 claim is not supported by this figure alone. Rerunning the sweep for all three two-agent methods, plus a finer sweep of coordination demands (corridor width, time pressure, asymmetric goals), is the natural follow-up.
\end{enumerate}
